...
namespace MovieAgent.Server.Library
{
	...
	public class BasicWebCrawler
	{
		...
		public static void Fork(string Query)
		{
			var HTTP_HOST = Native.SuperGlobals.Server[Native.SuperGlobals.ServerVariables.HTTP_HOST];
			var SERVER_PORT = int.Parse(Native.SuperGlobals.Server[Native.SuperGlobals.ServerVariables.SERVER_PORT]);
			var REQUEST_URI = Native.SuperGlobals.Server[Native.SuperGlobals.ServerVariables.REQUEST_URI];

			var i = REQUEST_URI.IndexOf("?");
			if (i >= 0)
				REQUEST_URI = REQUEST_URI.Substring(0, i);

			REQUEST_URI += "?" + Query.URLEncode();
			...
			var fork = new BasicWebCrawler(HTTP_HOST, SERVER_PORT);
			fork.DiscardResponse = true;
			fork.Crawl(REQUEST_URI);
		}
